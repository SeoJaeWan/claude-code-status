...
- 작업:
    ...
    - `/plugin-id:install-global` skill을 만들어 `~/.claude/settings.json`의 `statusLine.command`를 안정 경로 launcher로 설정한다.
...
- 실행:
    ...
    - `/plugin-id:install-global`
    - `/plugin-id:doctor`
- 완료조건:
    - plugin이 user scope로 설치되고, `install-global` 실행 후 전역 status line이 persistent launcher를 가리킨다.
